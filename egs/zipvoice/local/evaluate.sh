...
if [ $start_stage -le 1 ] && [ $end_stage -ge 1 ]; then 
        ...
        python3 -m zipvoice.evaluate.evaluate_sim \
        ...
        echo "-----Computing WER-----"
        python3 -m zipvoice.evaluate.evaluate_wer_seedtts \
                --test-list ${test_list} \
                --wav-path ${wav_path} \
                --lang "en" 

        echo "-----Computing UTSMOS-----"
        python3 -m zipvoice.evaluate.evaluate_utmos \
                --wav-path ${wav_path} \
...
if [ $start_stage -le 3 ] && [ $end_stage -ge 3 ]; then 
        ...
        echo "-----Computing WER-----"
        python3 -m zipvoice.evaluate.evaluate_wer_hubert \
                --test-list ${test_list} \
                --wav-path ${wav_path} \
